...
// Evaluate a candidate (heuristic breakdown)
const evalJson = evaluate_candidate(game, JSON.stringify(placements), { difficulty: 'medium' });
console.log('eval', JSON.parse(evalJson));
```
...
const { eval: evaluation } = await call('evaluate_candidate', {
  placements: [{ x: 3, y: 3, kind_id: 'A' }],
  difficulty: 'medium',
});
