...
Phase 14 introduces first-class persistence for TileTangle. Every `GameState` can now be exported to a
stable snapshot (JSON or CBOR) together with an append-only event log and Zobrist hash of the
position. Snapshots capture the complete material state (board, racks, bag RNG, scores, turn order)
so you can resume play, build undo/redo stacks, or ship reproducible fixtures in tests.
