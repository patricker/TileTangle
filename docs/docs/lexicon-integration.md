...
- Prebuilt caches for the docs:
  - `docs/static/dictionaries/TWL06.fst` (finite-state set) — generated by `make wasm`.
  - `docs/static/dictionaries/TWL06.gaddag.cbor` (GADDAG automaton) — generated by `make wasm`.
    Compressed variants (`.cbor.zst` / `.cbor.gz`) are supported by the loader.

Reproduce locally

```bash
# From repo root — build FST + GADDAG caches next to the text list
make wasm

# Or generate a GADDAG cache with custom options (compressed for smaller size)
cargo run --bin gaddag_cache -- \
  docs/static/dictionaries/TWL06.txt \
  docs/static/dictionaries/TWL06.gaddag.cbor.zst \
  --case-fold --norm nfc --zstd
```

Notes:
- `gaddag_cache` honors normalization and tokenization via flags. Use `--tokenizer char` to split by codepoints instead of graphemes.
- File sizes vary by list and options. The CBOR image is a direct dump of the in-memory structure for fast loading.
