...
For every candidate returned by `generate_moves` we compute:

- **Raw score** — the immediate points from `CrosswordRules`.
- **Rack leave** — a configurable lookup table (default Scrabble-inspired values) that rewards
...
- **Endgame penalty** — when the bag is empty we subtract the face value of tiles left on the rack
...
nodes/time, inject noise for easier levels, and more.
