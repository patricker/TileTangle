...
func _refresh_symbols():
	...
	if parsed == null:
		return
	var slice_h: int = max(1, int(parsed["height"]) / max(1, D))
...
func play_sample_word() -> void:
	...
	if board_json == "":
		return
	var board: Dictionary = JSON.parse_string(board_json)
	if board == null:
		return
	var rack_json: String = eng.get_rack_json()
	if rack_json == "":
		return
	var rack: Array = JSON.parse_string(rack_json)
	if rack == null:
		return
	var tiles := []
	for entry in rack:
	...
	var slice_h: int = max(1, full_h / max(1, D))
	var cx := int(W / 2)
	var cy := int(H / 2)
	var placements := []
