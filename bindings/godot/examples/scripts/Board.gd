    ...
    for y in range(height):
      for x in range(width):
        ...
        if nx >= 0 and nx < width and ny >= 0 and ny < height:
          edges.append({"a": y*width + x, "b": ny*width + nx, "dir": "E"})
        # NE neighbor
        nx = x + offset
        ny = y - 1
        if nx >= 0 and nx < width and ny >= 0 and ny < height:
          edges.append({"a": y*width + x, "b": ny*width + nx, "dir": "NE"})
        # SE neighbor
        nx = x + offset
        ny = y + 1
        if nx >= 0 and nx < width and ny >= 0 and ny < height:
          edges.append({"a": y*width + x, "b": ny*width + nx, "dir": "SE"})
...
func _refresh_board():
  ...
  if board_json == "":
    return
  var parsed: Dictionary = JSON.parse_string(board_json)
  if parsed == null:
    return
  var idx := 0
  for y in range(height):
    for x in range(width):
      ...
      if node is Button:
        var symbol: String = String(parsed["rows"][y][x])
        node.text = symbol
  ...
  for it in staged:
    var sx := int(it["x"])
    var sy := int(it["y"])
    var sym: String = String(kind_to_symbol.get(String(it["kind_id"]), String(it["kind_id"])) )
    var i := sy * width + sx
  ...
  if staged.size() > 0:
    var preview: Variant = _preview_json()
    if preview != null:
...
func _update_score_summary() -> int:
  ...
  for i in range(players.size()):
    ...
    if i == 0:
      ...
      cpu = score
  var turn_text := "Your turn" if to_move == 0 else "CPU thinking"
  if score_summary_label:
...
func _preview_json():
  ...
  if res == "":
    return null
  var parsed: Dictionary = JSON.parse_string(res)
  if parsed == null:
    return null
  var total := int(parsed.get("total", 0))
  var main_word := String(parsed.get("main_word", ""))
  var main_score := int(parsed.get("main_score", 0))
  var cross_words: Array = parsed.get("cross_words", [])
  var cross_sum: int = 0
  for cw in cross_words:
    if cw.size() >= 2:
      cross_sum += int(cw[1])
  var label: Label = get_node("ScoreLabel") as Label
  if label:
      var valid := bool(parsed.get("valid", false))
      var label_text := ""
      if valid:
        label_text = "Preview: %s total=%d (main=%d +cross=%d)" % [main_word, total, main_score, cross_sum]
      else:
        label_text = "Preview: invalid"
...
func _refresh_rack():
  ...
  if rack_json == "":
    return
  var parsed: Array = JSON.parse_string(rack_json)
  if parsed == null:
    return
  var bar := HBoxContainer.new()
  bar.name = "RackBar"
  ...
  for item in parsed:
    var btn := Button.new()
    var label: String = String(item["symbol"]) if item.has("symbol") else String(item["kind_id"])
    var score: int = int(item.get("score", 0))
    btn.text = "%s\n%d" % [label, score]
    var kid: String = String(item["kind_id"])
...
func _update_score_overlay(score_json: String):
  var parsed: Dictionary = JSON.parse_string(score_json)
  if parsed == null:
    return
  var total := int(parsed.get("total", 0))
  var main_word := String(parsed.get("main_word", ""))
...
func _play_cpu_move():
  ...
  if best_json == "null":
    ...
    _reset_cpu_button()
    return
  var parsed: Dictionary = JSON.parse_string(best_json)
  if parsed == null:
  ...
  if preview_json != "":
    var preview: Dictionary = JSON.parse_string(preview_json)
    if preview != null:
      last_main = preview.get("main_cells", [])
      last_cross = preview.get("cross_cells", [])
