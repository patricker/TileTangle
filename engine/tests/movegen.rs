...
fn generate_with_multichar_tile() {
    ...
    let tileset = eng::Tileset { tile_kinds: vec![
        eng::TileKind { id: "QU".into(), symbol: "QU".into(), score: 10, is_blank: false, aliases: vec![] },
        eng::TileKind { id: "A".into(), symbol: "A".into(), score: 1, is_blank: false, aliases: vec![] },
    ] };
    let mut counts = std::collections::HashMap::new();
    ...
    let mut st = eng::GameState::new(&cfg, 2).unwrap();
    st.dictionary = Some(Box::new(eng::FstDictionary::from_words(vec!["QUA".to_string()], true)));
    ...
    let center = eng::CrosswordRules::center_cell(&st.board.geom);
    let right = st.board.geom.to_cell_id(eng::Coord2D { x: st.board.geom.from_cell_id(center).unwrap().x + 1, y: st.board.geom.from_cell_id(center).unwrap().y }).unwrap();
    let mv = eng::MoveDraft { placements: vec![(center, eng::Tile { kind_id: "QU".into(), mark: None }), (right, eng::Tile { kind_id: "A".into(), mark: None })] };
    let v = rules.validate(&st, &mv).unwrap();
    let sc = rules.score(&st, &v);
    // direct scoring sanity
    assert!(sc.total >= 0);
    let moves = eng::generate_moves(&st, &rules, &rack, 7);
    assert!(moves.iter().any(|cm| cm.word == "QUA" && cm.placements.len() == 2));
}
