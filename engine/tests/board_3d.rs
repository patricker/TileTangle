...
fn cfg_wh(w: u32, h: u32) -> eng::GameConfig {
    ...
    let mut counts = std::collections::HashMap::new(); counts.insert("A".into(), 50); counts.insert("B".into(), 50);
...
fn apply_3d_overlay(st: &mut eng::GameState, w:i32,h:i32,d:i32) {
    ...
    for z in 0..d { for y in 0..h { for x in 0..w { nodes.push(eng::Coord2D { x, y: y + z*h }); } } }
...
fn validate_z_line_and_score() {
    let w=3; let h=3; let d=2; // depth 2
    ...
    let id0 = st.board.geom.to_cell_id(eng::Coord2D { x:1, y:1 }).unwrap();
    let id1 = st.board.geom.to_cell_id(eng::Coord2D { x:1, y:1 + h as i32 }).unwrap();
    let mv = eng::MoveDraft { placements: vec![
        (id0, eng::Tile { kind_id:"A".into(), mark: None }),
        (id1, eng::Tile { kind_id:"B".into(), mark: None }),
    ]};
...
fn bonus_applies_in_3d() {
    let w=3; let h=3; let d=2;
    let mut st = eng::GameState::new(&cfg_wh(w, h*d as u32), 2).unwrap();
    apply_3d_overlay(&mut st, w as i32, h as i32, d as i32);
    // Set a double word on the second cell
    let id1 = st.board.geom.to_cell_id(eng::Coord2D { x:1, y:1 + h as i32 }).unwrap();
    st.board.bonuses.insert(id1, eng::Bonus { letter_mul: 1, word_mul: 2, tags: std::collections::BTreeSet::new() });
    let rules = eng::CrosswordRules::default();
    let id0 = st.board.geom.to_cell_id(eng::Coord2D { x:1, y:1 }).unwrap();
    let mv = eng::MoveDraft { placements: vec![
        (id0, eng::Tile { kind_id:"A".into(), mark: None }),
