...
fn stress_many_anchors_long_rack_small_board() {
    ...
    let mut counts = std::collections::HashMap::new(); counts.insert("A".to_string(), 100); counts.insert("B".to_string(), 100);
    let cfg = eng::GameConfig { tileset, rack_size: 7, board_layout: eng::RectBoardLayout { width: 7, height: 7 }, ruleset_id: "cross".into(), dictionary_id: "en".into(), rng_seed: 1, tile_counts: counts };
    let mut st = eng::GameState::new(&cfg, 2).unwrap();
    // Dict allows a few sequences to avoid explosion
    let dict = eng::FstDictionary::from_words(vec!["AA".to_string(), "ABA".to_string(), "BAB".to_string(), "AB".to_string(), "BA".to_string()], true);
    ...
    let mut pre = vec![c];
    pre.push(st.board.geom.to_cell_id(eng::Coord2D { x: cc.x - 1, y: cc.y }).unwrap());
    pre.push(st.board.geom.to_cell_id(eng::Coord2D { x: cc.x + 1, y: cc.y }).unwrap());
    pre.push(st.board.geom.to_cell_id(eng::Coord2D { x: cc.x, y: cc.y - 1 }).unwrap());
    pre.push(st.board.geom.to_cell_id(eng::Coord2D { x: cc.x, y: cc.y + 1 }).unwrap());
    for id in pre {
        st.board.cells[id.0 as usize].stack.push(eng::Tile { kind_id: "A".into(), mark: None });
    }
    // Long rack
    let rack = vec!["A".to_string(), "B".to_string(), "A".to_string(), "B".to_string(), "A".to_string(), "B".to_string(), "A".to_string()];
