...
use crate::{
    Tile, TileKind,
    dict::{GaddagCursor, GaddagDictionary},
    game::GameState,
    ...
    rules::{
        Rules,
        crossword::{CrosswordRules, ValidatedMove},
    },
};
...
pub fn generate_moves(
    ...
    fn dfs_right(
            ...
            if let Some(t) = cell.stack.last() {
                ...
                if let Some((gd, node)) = gaddag {
                    let ns = {
                        ...
                        if gd.forward.case_fold {
                            s = s.to_lowercase();
                        }
                        s
                    };
                    if let Some(n2) = gd.step_symbol(node, &ns) {
                        gaddag = Some((gd, n2));
                    } else {
                        return;
                    }
        ...
        if state.board.cells[id.0 as usize].stack.is_empty() {
            ...
            for (kind_id, cnt) in rack.clone() {
                ...
                let vtiles = {
                    let mut count = 0usize;
                    let mut c = state.board.geom.from_cell_id(id).unwrap();
                    let (dx, dy) = pdir;
                    loop {
                        let prev = Coord2D {
                            x: c.x - dx,
        ...
        if let Some(left_id) = state.board.geom.to_cell_id(left) {
            ...
            for sym in blank_syms.into_iter() {
                ...
                if let Some(bid) = chosen_blank {
                    ...
                    if let Some((gd, base2)) = next_g_pre2 {
                        ...
                        if let Some(n2) = gd.step_symbol(base2, &ns) {
            ...
            for (kind_id, cnt) in rack.clone() {
                ...
                if let Some((gd, base)) = next_g_pre {
                    let ns = norm_sym(gd, sym);
                    if let Some(n2) = gd.step_symbol(base, &ns) {
    ...
    if let Some(dict) = &state.dictionary
        && let Some(gd) = dict.as_any().downcast_ref::<GaddagDictionary>()
    {
        gaddag_pre_seed = Some(gd);
    }

    for &a in &anchors {
        if !state.board.cells[a.0 as usize].stack.is_empty() {
            continue;
        }
...
fn generate_moves_graph_basic(
    ...
    fn allowed_for_cell_on_axis(
        ...
        let (left_syms, right_syms) = match (sides.first(), sides.get(1)) {
            ...
            (Some(&a), None) | (None, Some(&a)) => {
                let rs = collect_chain_symbols_from(state, a, cell, ctag);
                if rs.is_empty() {
            ...
            (Some(&a), Some(&b)) => {
                let ls = collect_chain_symbols_from(state, a, cell, ctag);
                let rs = collect_chain_symbols_from(state, b, cell, ctag);
    ...
    fn cross_checks_for_tag(
        ...
        for (idx, cell) in state.board.cells.iter().enumerate() {
            ...
            for ctag in perp {
                ...
                allowed = allowed.intersection(&on_axis).cloned().collect();
            }
    ...
    fn explore_segment(
        segment: &[CellId],
        idx: usize,
        rack_counts: &mut Map<String, usize>,
        ...
        if idx == segment.len() {
            ...
            let draft = crate::game::MoveDraft {
    ...
    for &anchor in anchors {
        ...
        for (_n, tag) in state.board.geom.neighbors_with_tags(anchor) {
