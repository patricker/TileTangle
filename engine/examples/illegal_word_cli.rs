...
fn main() {
    ...
    let mv = MoveDraft { placements: vec![(c, Tile { kind_id: "A".into(), mark: None })] };
    let v = rules.validate(&state, &mv).unwrap();
    let sc = rules.score(&state, &v);
    if sc.main_score < 0 { eprintln!("Rejected: word not in dictionary"); } else { println!("Accepted: total {}", sc.total); }
}
