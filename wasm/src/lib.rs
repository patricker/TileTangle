...
struct JsConfig {
    ...
    rng_seed: u64,
    #[serde(default)] tile_counts: std::collections::HashMap<String, u32>,
    #[serde(default)] free_word_mode: bool,
}
...
pub fn new_game(config_json: &str, players: usize) -> Result<JsGame, JsValue> {
    ...
    let tileset = engine::Tileset { tile_kinds: cfg.tileset.tile_kinds.into_iter().map(|k| engine::TileKind { id: k.id, symbol: k.symbol, score: k.score, is_blank: k.is_blank, aliases: k.aliases }).collect() };
    let eng_cfg = engine::GameConfig {
        tileset,
        rack_size: cfg.rack_size,
        board_layout: engine::RectBoardLayout { width: cfg.board_layout.width, height: cfg.board_layout.height },
    ...
    let rules = engine::CrosswordRules { free_word_mode: cfg.free_word_mode, ..Default::default() };
...
pub fn play_move(game: &mut JsGame, placements_json: &str) -> Result<JsValue, JsValue> {
    ...
    for p in placements {
        let cid = game.state.board.geom.to_cell_id(engine::Coord2D { x: p.x, y: p.y }).ok_or_else(|| to_js_err("invalid coordinates"))?;
    ...
    if score.main_score < 0 { return Err(to_js_err("invalid word(s)")); }
    game.rules.commit(&mut game.state, validated, &score).map_err(to_js_err)?;
    Ok(JsValue::from_str(&serde_json::to_string(&score_to_json(&score)).unwrap()))
}
...
fn to_js_err<E: std::fmt::Display>(e: E) -> JsValue { JsValue::from_str(&format!("{}", e)) }
